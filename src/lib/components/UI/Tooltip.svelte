...
<div class="inline-block {customClass}">
	...
	{#if isIconHovered}
		<div
			...
			class="w-60 absolute top-1/2 -right-6 translate-x-full -translate-y-1/2 p-3 rounded-lg
			{isDark ? 'bg-txt_main/80 backdrop-blur-xl' : 'bg-neutral-600'}"
		>
			{description}
			<div class="absolute w-0 h-0 left-0 top-1/2 -translate-x-full -translate-y-1/2 border-y-transparent border-l-transparent border-r-neutral-600 border-y-8 border-r-8 border-l-0 {isDark ? 'border-r-neutral-800/80' : 'border-r-neutral-600'}"></div>
		</div>
	{/if}
</div>